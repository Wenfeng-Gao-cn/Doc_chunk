...
class KnowledgeTreeModifier:
    ...
    def modify_knowledge_tree(self, modification_list: ModificationList, 
        ...
        # 逐条执行修改指令
        for i, operation in enumerate(modification_list.operations):
            try:
    ...
    def _execute_action(self, tree: KnowledgeTree, operation):
        """执行单个修改操作"""
        action_type = operation.modification_action
        target_path = operation.target_path
        content = operation.modification_content
        
        if not target_path:
            raise ValueError("Missing target_path in operation")
        ...
        if action_type == "add":
            ...
            raise ValueError(f"Unknown modification_action: {action_type}")
    
    def _add_node(self, tree: KnowledgeTree, target_path: str, content: Any):
        """添加节点"""
        if content is None:
    ...
    def _get_node_by_path(self, tree: KnowledgeTree, path_parts: List[str]) -> KnowledgeNode:
        ...
        for i, part in enumerate(path_parts[1:], 1):  # 跳过 'root'
            if part == "children":
                ...
                # 处理数组索引
                index = int(part[1:-1])
                if current_node.children is None or index >= len(current_node.children):
                    raise ValueError(f"Invalid path: index {index} out of range")
    ...
    def _create_node_from_content(self, content: Any) -> KnowledgeNode:
        """从内容创建节点"""
        if isinstance(content, dict):
            ...
            raise ValueError("Content must be a dictionary for node creation")
...
if __name__ == "__main__":
    ...
    test_operations = [
        ...
        ModificationOperation(
            ...
            target_path="root.children[0].content",
            modification_content="修改后的第一章内容",
            reason="更新第一章内容"
        ),
        ModificationOperation(
            is_modification_needed=True,
            modification_action="add",
            target_path="root.children[0].children",
            modification_content={
                "title": "第二节",
                "content": "第二节的内容",
                "children": None
    ...
    test_modification_list = ModificationList(operations=test_operations)
